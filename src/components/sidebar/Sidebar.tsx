...
const Sidebar: React.FC = () => {
  ...
  const onFilter = (e: MouseEvent<HTMLButtonElement>) => {
    switch (e.currentTarget.textContent) {
      case "Men's clothing":
        dispatch(
          fetchProducts(
            `category/${e.currentTarget.textContent.toLowerCase()}`,
          ),
        );
        break;
      case "Jewelery":
  ...
  return (
    <div className=" flex items-center justify-between flex-col gap-4 mdx:flex-row">
      <div className=" flex flex-wrap justify-center items-center gap-[10px]">
        {buttons.map(({ text, id }) => {
          return (
            <Button
              handler={onFilter}
              secondHandler={onActiveButton}
              key={id}
              className={` text-black text-[16px] font-medium border-[1px] border-black rounded-[8px] px-[4px] py[4px] ${
                activeTab === text ? "bg-[#F9A43F]" : ""
              }`}
            >
              {text}
            </Button>
      ...
      <input
        className=" w-[280px] h-[40px] px-[10px] py-[10px] border-[1px] border-black rounded placeholder:text-[#929292] placeholder:font-medium placeholder:text-[14px]"
